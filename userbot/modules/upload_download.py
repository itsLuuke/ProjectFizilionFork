...
@register(pattern=r"^{trg}dl(?: |$)(.*)".format(trg=trgg), outgoing=True)
...
@register(pattern=r"^{trg}ul (.*)".format(trg=trgg), outgoing=True)
...
@register(pattern=r"^{trg}download(?: |$)(.*)".format(trg=trgg), outgoing=True)
async def download(target_file):
    """ For .download command, download files to the userbot's server. """
    await target_file.edit("Processing ...")
    input_str = target_file.pattern_match.group(1)
...
@register(pattern=r"^{trg}uploadir (.*)".format(trg=trgg), outgoing=True)
...
@register(pattern=r"^{trg}upload (.*)".format(trg=trgg), outgoing=True)
...
@register(pattern=r"^{trg}uploadas(stream|vn|all) (.*)".format(trg=trgg), outgoing=True)
